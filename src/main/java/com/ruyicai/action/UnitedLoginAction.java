...
public class UnitedLoginAction extends BaseAction {
	...
	public String alipayHandyLogin(){
		...
		try {
			...
			String return_url = retUrl3Gcaipiao+";jsessionid="+request.getSession(true).getId();
	...
	public String alipayUnitedLogin(){
		...
		try {
			...
			if(CommonUtil.getBackValue("errorCode", obj).equals(LotErrorCode.NEW_OK)){
				...
				response.getWriter().print("<script language='javascript'>window.location.href='/unitedLogin/setNickName_logistics.jsp'</script>");
			}
			return null;
	...
	public boolean  getBigLogin(String userno,HttpServletRequest request) throws Exception{
		JSONObject loginfo = CommonUtil.loginForUserno(userno, request, response);
			...
			if(!bindObj.isNullObject()){
				if(CommonUtil.getBackValue("errorCode", bindObj).equals(LotErrorCode.NEW_OK)){
					...
					if(getBigLogin(tuser.getUSERNO(),request)){
						...
						response.getWriter().print("http://www.ruyicai.com/index.html");
						return null;
					}else{
						session.setAttribute("message", MessageUtil.TIAW_login_ExceptionMsg);
						response.getWriter().print("/unitedLogin/setNickName_logistics.jsp");
						return null;
		...
		try{
			...
			if(getBigLogin(tuser.getUSERNO(),request)){
				if("null".equals(tuser.getUSERNAME()) || "".equals(tuser.getUSERNAME().trim())){
					...
					response.getWriter().print("<script language='javascript'>window.location.href='http://www.ruyicai.com/index.html'</script>");
				}
				return null;
			}else{
				session.setAttribute("message", MessageUtil.TIAW_login_ExceptionMsg);
			...
			if(tuser != null){
				...
				if(!bindObj.isNullObject()){
					...
					if(CommonUtil.getBackValue("errorCode", bindObj).equals(LotErrorCode.NEW_OK)){
						...
						response.getWriter().print("success");
						//response.getWriter().print("/chlw/function/user/setNickName_logistics.jsp");
						return null;
 ...
 */
	...
	public String qqUnitedLogin(){
		...
		try {
				...
				//JSONObject jsonUserInfoes = JSONObject.fromObject(getUserInfoes);
