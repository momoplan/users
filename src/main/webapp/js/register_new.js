...
regArray[0]= "/register/register_userInfo.jsp";
regArray[1]= "/register/register_Success.jsp"; 
regArray[2]= "/register/register_91_userInfo.jsp";
regArray[3]= "/register/register_91_Success.jsp"; 
/*******************offen.js***********************
...
// 投注时查询余额
	function getBalance(){
		$.ajax({
			type: "POST",
			url: "/ajax/selectAll!ajaxFindAccount",
			dataType:'json',
...
/**
  ...
  //直接加载jsp页面
	...
	function change(mag){
		var d=new Date();
		var imageUrl = "/common/image.jsp?id="+d.getDate()+Math.random();
...
//======================提示框end=============================	
  ...
  var nameflag=0;
	function checkUserName(){
		...
		$.ajax({
			url:'/function/validateAction!changeUserName',
			type:"POST",
			data:params,
			//async:false,
			dataType:'text',
...
function register_91(){
	...
	$.ajax({
		url	: '/function/registerAction!new_register',
		type:"POST",
		data:params,
		async:false,
		dataType:'text',
		success:function(data){
			if(data =="cerSuccess"){
				getBalance();  
				//需要加跳转到第二个注册页面的地址
				reHtmljsp(2,'',true,'registerbody',false);
	...
	function checkMobile(){
		...
		/**$.ajax({
			url:'/chlw/function/tuserinfoAction!queryMobile?mobile='+Trim(tel),
			type:'POST',
			dataType:'text',
			error:function(){openAlert("网络出现异常");
	...
	function verifyuserinfo_91(){
		...
		var params ="isIe=中文";
		for(b in $.browser) if(b=='msie'){params=params+"&msie=1";};
		params =params+"&nickname="+nickname+"&name="+name+"&cardid="+cardid+"&tel="+mobile+"&email="+email+"&verifyCode="+verifyCode;
		if(nickname=="" && name=="" && cardid=="" && mobile=="" && email==""){
			...
			if(nickflag==1 && realflag==1 && certid==1 && telflag==1 && emailflag==1){
				...
				$.ajax({
					url	: '/function/validateAction!modifyUserInfo',
					type:"POST",//数据发送方式
					data:params,//参数
					...
					success:function(json){
						if(json=="success"){
							reHtmljsp(3,'',true,'registerbody',false);
						}else if(json=="failed"){
							openAlert("修改用户信息失败！");
	...
	function checkverifyCode(){
		...
		if(verifyCode==null || verifyCode == "undefind" || verifyCode ==""){
			addWrongImages('verifyCode','verifyCodeImage','verifyCodeTip','请输入验证码！');
			return false;
		}else if($("#verifyCode").length>0){
			var zhengshu = /^-?\d+$/;
			if(zhengshu.test(verifyCode)){
				addRightImages("verifyCodeImage","verifyCodeTip");
				return true;
			}
			addWrongImages('verifyCode','verifyCodeImage','verifyCodeTip','必须和您手机收到的保持一致！');	
			return false;
		}
	}